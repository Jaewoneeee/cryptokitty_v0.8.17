...
describe("KittyCore & SaleClockAuction", () => {
    ...
    describe("SaleClockAuction", () => {
        ...
        it("bid gen0 1", async () => {
            ...
            const kittyCoreBal_bef = await ethers.provider.getBalance(
                kittyCore.address
            );
            ...
            const kittyCoreBal_aft = await ethers.provider.getBalance(
                kittyCore.address
            );
            console.log(`KittyCore balance: ${kittyCoreBal_aft}`);
            const saleClockAuction_aft = await ethers.provider.getBalance(
        ...
        it("bid gen0 2", async () => {
            ...
            console.log(`KittyCore balance: ${kittyCoreBal_bef}`);
            const saleClockAuction_bef = await ethers.provider.getBalance(
                saleClockAuction.address
            );
        ...
        it("createAuction should fail if it's not called by nonFungibleContract (KittyCore)", async () => {
            ...
            ).to.be.revertedWith("you're not the nonFungibleContract");
        });
    });
    ...
    describe("User Create Auction", () => {
        ...
        it("bid by signer3", async () => {
            ...
            const signer2Bal_bef = await ethers.provider.getBalance(
                signers[2].address
            );
            console.log(`signer2 balance: ${signer2Bal_bef}`);
            const signer3Bal_bef = await ethers.provider.getBalance(
                signers[3].address
            );
            ...
            const signer2Bal_aft = await ethers.provider.getBalance(
                signers[2].address
            );
            console.log(`signer2 balance: ${signer2Bal_aft}`);
            const signer3Bal_aft = await ethers.provider.getBalance(
                signers[3].address
            );
            console.log(`signer3 balance: ${signer3Bal_aft}`);

            console.log(
                `signer2 balance diff: ${signer2Bal_aft.sub(signer2Bal_bef)}`
            );
        });
    });
    ...
    describe("WithdrawBalance", () => {
        it("Withdraw balance from SaleClockAuctino to KittyCore", async () => {
            ...
            expect(kittyCoreBal_aft.sub(kittyCoreBal_bef)).to.equal(
                saleClockAuction_bef
            );
        });
        ...
        it("Withdraw balance from KittyCore to CFO", async () => {
            ...
            console.log(
                `KittyCore balance diff: ${kittyCoreBal_aft.sub(
                    kittyCoreBal_bef
                )}`
            );
